...
masks = xr.DataArray(cloud_shadow_masks, dims=['time', 'y', 'x'], coords=[np.ones(t_series['reflectance'].shape[0]), np.ones(t_series['reflectance'].shape[1]), np.ones(t_series['reflectance'].shape[2])])
masks.assign_coords(time=coords_copy['time'], x=coords_copy['x'], y=coords_copy['y'])
masks = masks.astype(np.int16)
masks.rio.to_raster("/home/rave/pyatsa/cfg/atsa_results/citymasks.tif")
