...
hot_t_series, intercepts_slopes = masking.compute_hot_series(
    t_series['reflectance'], configs.rmin, configs.rmax)
...
refined_masks = masking.apply_upper_thresh(t_series['reflectance'], hot_t_series, upper_thresh_arr,
...
for i in np.arange(refined_masks.shape[0]):
    ...
    refined_masks[i] = np.where((t_series['reflectance'][:, :, 3, i] < 2000) & (
...
h_high, h_low = masking.cloud_height_min_max(
    t_series['angles'], configs.longest_d, configs.shortest_d)
h_ranges = masking.cloud_height_ranges(h_high, h_low)
shift_coords = masking.shadow_shift_coords(h_ranges, t_series['angles'])
...
clearest_land_nir, clearest_land_index = masking.min_cloud_nir(
    potential_shadow_masks, t_series['reflectance'])
gains_biases = masking.gains_and_biases(
    potential_shadow_masks, t_series['reflectance'][:,:,:,3], clearest_land_nir, clearest_land_index)
shadow_inds = masking.shadow_index_land(
    potential_shadow_masks, t_series['reflectance'], gains_biases)
